...
const PostRequestModal = ({ open, onClose, onSubmit, form, onChange, categories, deadlines }) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center backdrop-blur-sm bg-transparent">
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-lg relative">
        ...
        <form className="flex flex-col gap-4" onSubmit={onSubmit}>
          <input type="text" name="title" value={form.title} onChange={onChange} required placeholder="Help Needed" className="px-4 py-2 rounded-lg border border-gray-300 bg-gray-50 text-gray-900 focus:outline-none" />
          ...
          <select name="deadline" value={form.deadline} onChange={onChange} required className="px-4 py-2 rounded-lg border border-gray-300 bg-gray-50 text-gray-900 focus:outline-none">
          ...
          <button type="submit" className="w-full py-2 rounded-lg font-bold bg-[var(--color-accent)] text-[var(--color-surface)] shadow-lg hover:opacity-90 transition">Submit Request</button>
        </form>
      </div>
    </div>
  );
